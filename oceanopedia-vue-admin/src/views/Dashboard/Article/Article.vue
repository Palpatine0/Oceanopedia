<template>
    <v-app style="padding: 20px">
        ...
        <v-sheet :color="`gainsboro ${true ? 'darken-2' : 'lighten-4'}`" class="pa-3">
            ...
            <v-row>
                ...
                <v-col cols="6" md="2"  >
                    <v-progress-circular :rotate="90" :size="230" :width="35" :value="article.views !== 0 ? parseFloat((article.likes / article.views) * 100).toFixed(2) : 0" color="#5aaeaa" style="margin-left: -66px">
                        <span style="font-size: 20px;font-weight: bold">{{ article.views !== 0 ? parseFloat((article.likes / article.views) * 100).toFixed(2) : 0 }}%</span>
                    </v-progress-circular>
                </v-col>
            </v-row>
        </v-container>
